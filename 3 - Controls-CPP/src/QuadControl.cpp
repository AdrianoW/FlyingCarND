...
V3F QuadControl::BodyRateControl(V3F pqrCmd, V3F pqr)
{
  ...
  momentCmd = rate_error*kpPQR*MOI;
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  float p = z_err * kpPosZ;
  
  float d = (velZCmd - velZ) * kpVelZ + velZ;
  
  integratedAltitudeError += z_err * dt;
  float i = integratedAltitudeError * KiPosZ;
  
  
  float u_1 = p + d + i+ accelZCmd;
  
  thrust = mass *( CONST_GRAVITY - u_1)/R(2,2);
...
V3F QuadControl::LateralPositionControl(V3F posCmd, V3F velCmd, V3F pos, V3F vel, V3F accelCmdFF)
{
  ...
  //Limit speed
  float velocity_norm = sqrt(velocity_cmd.x*velocity_cmd.x +
                             velocity_cmd.y*velocity_cmd.y);
  
  if (velocity_norm > maxSpeedXY) {
    velocity_cmd = velocity_cmd*maxSpeedXY/velocity_norm;
  }
  
    
  accelCmd = accelCmdFF + kpPosXY*(posCmd-pos) + kpVelXY*(velCmd-vel);
  
  float accel_norm = sqrt(accelCmd.x*accelCmd.x +
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
  ...
  float yawRateCmd=0;
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  float yaw_cmd = fmodf(yawCmd, 2.0f*M_PI);
